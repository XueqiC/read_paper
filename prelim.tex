...
others. We measure this effect with one-hot allocations:}
...
\rev{Two diagnostics read off $\mathbf{T}(B)$ quantify the waste.
\emph{Diminishing returns} compares the marginal target gain of
20M$\rightarrow$80M with that of 80M$\rightarrow$150M: for the
strongest-improving targets the first is consistently larger
(Figure~\ref{fig:diminishing_returns}), i.e., the target saturates.
\emph{Negative spillover} measures the drop on non-target capabilities as the
target's budget grows, and it rises with budget
(Figure~\ref{fig:rising_spillover}). Beyond a capability-specific knee point,
extra tokens therefore buy little target benefit while amplifying
cross-capability degradation. Neither effect is new in isolation: off-target
drops are a form of catastrophic
forgetting~\citep{kirkpatrick2017overcoming,luo2023empirical}, and saturating
gains are ordinary diminishing returns. What allocation can exploit is their
\emph{structure}: transfer is signed and source-specific, it changes with
budget, and the two effects interact, so a token's marginal value depends on
where the budget has already gone.}
...
formalize this as a budget allocation problem across interdependent capabilities:}
