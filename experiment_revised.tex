...
$150$M remain significant (Figure~\ref{fig:signif}a).}
...
\rev{Appendix Table~\ref{tab:scaling} states the same in absolute points:
between the two budgets ReAD gains the most per additional $10$M tokens on six
of the eight capabilities and on average. Conversely, interpolating each
...
at $150$M (Appendix Table~\ref{tab:equiv_budget}). This token saving is an
...
(Figure~\ref{fig:signif}a).}

\begin{figure}[t]
  \centering
  \begin{subfigure}[b]{0.58\linewidth}
    \centering
    \includegraphics[height=108pt]{figure/rq3_ablation_avgmetric_20m_150m.pdf}
    \caption{Component ablation}
    \label{fig:rq3_ablation}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.38\linewidth}
    \centering
    \includegraphics[height=108pt]{figure/rqX_radar_sota_vs_read_150m.pdf}
    \caption{Capability-specific methods}
    \label{fig:radar}
  \end{subfigure}
  \vspace{-0.4em}
  \caption{\rev{(a) Ablation of ReAD components: average benchmark score over
  the eight capabilities at 20M and 150M tokens (higher is better). (b) ReAD
  versus step-by-step distillation, EoTD, and CodePLAN at 150M, each designed
  for one capability; axes show the fraction of the teacher--student gap closed
  per capability (higher is better).}}
...
(Figure~\ref{fig:signif}b--c; exact values in Appendix Table~\ref{tab:xstest}).
The four differences agree in direction, but with three seeds none of them
survives Holm correction (Figure~\ref{fig:signif}a), so we read this as
...
\begin{figure}[!t]
  ...
  \vspace{-1.4em}
  \caption{\rev{Statistical view of the results, computed from the reported means
  and standard deviations (three seeds). (a) ReAD's advantage in each comparison
  with an unadjusted 95\% Welch confidence interval; filled markers remain
  significant ($p<0.05$) after Holm correction within each group and budget
  (across all four tests for XSTest). Safe-refusal is shown as a reduction, so
  positive is better everywhere. (b--c) XSTest scores of ReAD and the best
  SFT/KD baseline (mean $\pm$ std). Exact values and $t$ statistics are in
  Appendix Tables~\ref{tab:xstest} and~\ref{tab:signif}; per-seed scores will be
  released with the code so that paired significance tests can be run on them.}}
  \label{fig:signif}
  \vspace{-0.6em}
\end{figure}
