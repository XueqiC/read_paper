...
transfer beyond capability-aligned benchmarks\rev{. We close with a significance
analysis of every reported comparison.}
...
advantage does not rest on the costs that we exclude from the budget $B$.}
...
seed standard deviations of $0.3$--$0.4$; Welch tests in Table~\ref{tab:signif}), as every allocation baseline already
...
\rev{We remove one component at a time with everything else fixed: without the
requirement identifier the allocation is uniform, without adaptation it is
fixed, and without interaction awareness the spillover penalty is dropped.
Figure~\ref{fig:rq3_ablation} shows that identification and adaptation drive
the largest gains, while interaction modeling adds a further improvement by
...
(Table~\ref{tab:xstest}); lower safe-refusal and higher unsafe-refusal are
better.}
...
\rev{Table~\ref{tab:signif} applies unpaired two-sided Welch $t$-tests (three
seeds per method) to every comparison above, using the reported means and
...
correction, and the XSTest safe-refusal improvements are not significant at
this sample size. Per-seed scores will be released with the code so that paired
tests can be run.}
