...
seed standard deviations of $0.3$--$0.4$; Welch tests in Appendix Table~\ref{tab:signif}), as every allocation baseline already
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.46\textwidth]{figure/rq3_ablation_avgmetric_20m_150m.pdf}
  \vspace{-0.4em}
  \caption{Ablation of ReAD components\rev{: average benchmark score over the eight
  capabilities at 20M and 150M tokens (higher is better).}}
...
allocation pipeline and no new capability head, ReAD improves both safe- and
unsafe-refusal metrics over the best SFT/KD baseline (Appendix
Table~\ref{tab:xstest}).}