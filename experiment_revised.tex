...
the $150$M setting. Reward regressors are two-layer MLPs trained on the 10\%
profiling split plus logged transitions, yielding $3.2$k--$4.9$k examples per
task. Proxy refresh adds 3.12\% wall-clock overhead, the full online loop adds
...
profile at $20$M tokens, consistently outperforming all six baselines. The gains
are especially pronounced on bottleneck capabilities such as Steerability, Math,
Tool Use, and LCU. The $150$M Llama results and Qwen results in
...
\begin{table}[t]
  ...
  \caption{\textbf{Budget-allocation diagnostics.} Panel (a) reports the 150M-token gain--spillover trade-off computed from the
capability transfer matrix: ReAD preserves near-best on-target gain while
substantially reducing off-target degradation and negative transfer.
Panel (b) compares matched-budget allocation schedules under the same generator,
showing that adaptive bandit allocation outperforms static and greedy schedules.}
...
\paragraph{Local proxy validation.}
...
uncertainty-aware bandit updates.
