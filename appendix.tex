...
\begin{figure}[t]
  ...
  that capability, read off Tables~\ref{tab:llama_20m}
  and~\ref{tab:llama_150m}. Error bars are the standard deviation over three
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.42\columnwidth]{figure/rqX_radar_sota_vs_read_150m.pdf}
  \caption{ReAD beats SOTA baselines that are specifically designed for reasoning, code, and math capabilities consistently\rev{, at the 150M-token budget}.}
  \label{fig:radar}
\end{figure}
...
\rev{\textbf{Significance and budget scaling of the reported numbers.}
Table~\ref{tab:signif} applies unpaired two-sided Welch $t$-tests (three seeds
per method) to the comparisons reported above, using only the printed means and
standard deviations, with Holm correction within each block. At $150$M every
gain over the allocation baselines of Table~\ref{tab:budget_diagnostics}(b)
remains significant after correction, and so do the Steerability and
Multilingual gains over the best one-hot baseline; at $20$M, with three seeds,
most per-capability differences are individually significant only before
correction. The XSTest safe-refusal improvements are not significant at this
sample size. Table~\ref{tab:scaling} reports how much each method gains per
additional $10$M tokens between the two budgets: ReAD has the largest marginal
gain on six of the eight capabilities and on average, which is the
budget-efficiency claim of RQ2 stated in absolute points.}
