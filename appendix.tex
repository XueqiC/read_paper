...
behaviors from their teachers, and cheaper distillation may ease building dual-use models. Appendix~\ref{sec:broader_impact} discusses these risks and possible mitigations.}
...
(accuracy, exact match, pass@1, or tool-call success rate) expressed in percentage points. LongBench uses its own per-task normalized score on the same $0$--$100$ scale. A capability score is the unweighted mean of its
...
student. We apply no min--max rescaling or gap-closure normalization to the reported tables or transfer matrices. A change of one point is therefore one percentage point of the underlying metric.}
...
\label{eq:taylor}
\end{equation}
which yields the local approximation in Eq.~\eqref{eq:local_linear}. This does not assume that the true sensitivity has no negative coordinates\rev{. We use the expansion only to motivate the training signal of Eq.~\eqref{eq:req_pretrain}. The priority vector $\mathbf{r}_\tau$ is a nonnegative, simplex-constrained allocation prior over capabilities, and we do not interpret it as an estimate of the sensitivity. Negative sensitivities enter through the signed capability changes that feed the reward.}
...
\caption{\rev{Target-specific distillation performance under a 20M-token budget (Qwen). ReAD has a higher mean score than all six one-hot baselines on every target.}}
...
\tiny
\setlength\tabcolsep{1pt}
\caption{\rev{Target-specific distillation performance under a 150M-token budget (Qwen). ReAD has a higher mean score than all six one-hot baselines on every target.}}
\begin{tabular}{lcccccccc}
\toprule
\textbf{Method} &
\textbf{General} &
...
settings, in line with the headroom analysis of Section~\ref{subsec:rq1}. Scaling the budget from 20M to 150M strengthens all methods. ReAD's average margin over the strongest baseline grows for Llama ($+2.0$ to $+2.6$ points) and narrows slightly for Qwen ($+1.8$ to $+1.6$ points), perhaps because Qwen's stronger student leaves less headroom. Qwen starts from a stronger student and teacher on every capability and reaches higher absolute scores after distillation. The strongest baseline for each capability and the top rank of ReAD stay the same, which suggests that ReAD's advantage does not depend on one LLM family.}
\rev{Figure~\ref{fig:gain_over_student} visualizes the same Llama numbers as
absolute gains over the initial student, next to the best one-hot baseline of each capability, so the per-capability margins can be compared directly.
...
Welch $t$ statistics shown in Figure~\ref{fig:signif}, with significance marks after Holm correction within each block and budget, as in the figure.}
...
ReAD is off-policy in this sense and treats the objective as a plug-in. Its contribution lies in allocating a fixed budget across capabilities, and it keeps a standard loss. A separate line of work optimizes the \emph{mixture} of training data. DoReMi~\citep{xie2023doremi} and RegMix~\citep{liu2024regmix} learn static
...
skills and updates skill-sampling weights online, including for a single target skill. ODM~\citep{albalak2023odm} treats data domains as arms of a multi-armed bandit rewarded by the training loss. Aioli~\citep{chen2025aioli} estimates
the parameters of a mixing law during training and uses them to adjust the
proportions. ReAD shares this online, feedback-driven view and applies it to target-oriented allocation in teacher-supervised distillation. A teacher generates supervision on the fly, the reward comes from measured changes in a capability profile, and regressions on prioritized capabilities receive an extra penalty. Finally, the off-target
degradation we measure is related to catastrophic forgetting in continual
fine-tuning~\citep{kirkpatrick2017overcoming,luo2023empirical}. ReAD adds no explicit anti-forgetting regularizer. Its reward penalizes measured regressions on prioritized capabilities (Eq.~\ref{eq:reward}).}
