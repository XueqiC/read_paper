...
fig, axes = plt.subplots(1, 2, figsize=(5.5, 3.1), sharey=True, gridspec_kw=dict(wspace=0.06))
fig.subplots_adjust(left=0.145, right=0.99, top=0.87, bottom=0.13)
y = np.arange(8)[::-1]
for ax, B in zip(axes, (20, 150)):
    ...
    for yy, a, b in zip(y, gb, gr):
        ax.plot([a, b], [yy, yy], color='#d9d8d2', lw=3.0, zorder=1, solid_capstyle='round')
    for yy, g, e, c in zip(y, gb, best[B][1], base_col):
        ax.errorbar(g, yy, xerr=e, fmt=mk, ms=5.5, mfc=c, mec=c, color=c, elinewidth=0.9, capsize=2.2,
                    capthick=0.9, zorder=3)
    ax.errorbar(gr, y, xerr=read[B][1], fmt=mk, ms=5.5, mfc=S.READ, mec=S.READ, color=S.READ, elinewidth=0.9,
                capsize=2.2, capthick=0.9, zorder=4)
    ax.set_title(f'Budget {B}M', fontsize=9, pad=4)
...
h = [Line2D([], [], color=S.METHOD[n], marker='o', ms=5.5, lw=0, label=n) for n in ('ReAD', 'Logit-KD', 'CoT-KD')]
h += [Line2D([], [], color='black', marker='o', mfc='white', ms=5.5, lw=0, label='20M'),
      Line2D([], [], color='black', marker='s', mfc='white', ms=5.5, lw=0, label='150M')]
fig.legend(handles=h, loc='upper center', ncol=5, fontsize=8.5, handlelength=1.0, columnspacing=1.2,
