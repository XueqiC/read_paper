...
for name in order[::-1]:
    ...
    ax.fill(a2, r2, color=c, alpha=0.08, zorder=2)
    ax.plot(a2, r2, color=c, lw=1.1 if name == 'ReAD' else 0.9, zorder=3)
...
h = [Line2D([], [], color=S.METHOD[n], lw=1.4, label=n) for n in order]
