...
\end{equation}
Thus, $\mathbf{r}_\tau$ is trained as a simplex-constrained surrogate for the beneficial part of the local utility sensitivity, e.g., $\mathbf{r}_\tau\approx\Pi_\Delta([\nabla_{\mathbf{s}}F_\tau(\mathbf{s})]_+)$. This does not assume that the true sensitivity has no negative coordinates; it only converts the sensitivity into a nonnegative budget-allocation prior.
...
ReAD runs for $T_{\mathrm{step}}=20$ decision steps. Thus, the bandit update, data resampling, and proxy refresh occur every $1.0$M tokens in the $20$M setting and every $7.5$M tokens in the $150$M setting. At step $t$, the context summarizes the task requirement, current student profile, remaining budget, and recent allocation history:
\begin{equation}
